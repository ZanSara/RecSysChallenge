...
def recommend(interactions_map, item_profiles, user_profiles, target_users):
    ...
    listed_interactions = interacting_items.groupby('user_id').apply( lambda group: group.drop_duplicates() )
    ...
    for u, user in enumerate(target_users):
        ...
        print(X)
        print("User {} gets these recs: {}".format( user, neigh.kneighbors(X=X, n_neighbors=5, return_distance=False) ))
        if u > 1: return
    print("Done!")
    return

def prepare_row(x):
    x.title = 0#str(x.title).split(',')
    x.tags = 0#str(x.tags).split(',')
    if x.country == 'at':
        x.country = 1
    elif x.country == 'ch':
        x.country = 2
        ...
        x.country = 3
    elif x.country == 'non_dach':
        x.country = 4
    else:
        print("Unknown country code: {}!".format(x.country))
    return x
